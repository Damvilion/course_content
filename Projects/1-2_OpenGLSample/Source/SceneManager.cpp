...
void SceneManager::LoadSceneTextures()
{
	...
	bReturn = CreateGLTexture(
		"textures/backdrop.jpg",//backdrop.jpg",
		"backdrop");
...
void SceneManager::SetupSceneLights()
{
	...
	m_pShaderManager->setVec3Value("directionalLight.ambient", 0.06f, 0.05f, 0.06f);
	m_pShaderManager->setVec3Value("directionalLight.diffuse", 0.5f, 0.5f, 1.0f);
	...
	// point light 3
	m_pShaderManager->setVec3Value("pointLights[2].position", -3.0f, 3.5f, 4.0f);
	m_pShaderManager->setVec3Value("pointLights[2].ambient", 0.05f, 0.05f, 0.05f);
	m_pShaderManager->setVec3Value("pointLights[2].diffuse", .9f, 0.8f, 0.7f);
	...
	m_pShaderManager->setVec3Value("pointLights[3].specular", 0.0f, 0.0f, 0.0f);
	...
	m_pShaderManager->setVec3Value("spotLight.diffuse", .8f, .8f, .8f);
...
void SceneManager::RenderCheeseWheel()
{
	...
	// enable the texture overlay
	SetShaderTextureOverlay("cheese_wheel_label");
	SetTextureUVScale(1.0, 1.0);
...
void SceneManager::RenderWineGlass()
{
	...
	/*********************************************************
	/  Draw the rounded part of the glass
	/********************************************************/
	/*** Set needed transformations before drawing the basic mesh ***/
...
void SceneManager::RenderWineBottle()
{
	...
	/*********************************************************
	/  Draw rim at very top
	/********************************************************/

	/*** Set needed transformations before drawing the basic mesh ***/

	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(.28f, 0.4f, 0.28f);
...
void SceneManager::RenderPlateAndKnife()
{
	...
	/*********************************************************
	/  Draw the cylinder base on the very bottom of the plate
	/********************************************************/
	...
	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(0.46f, 0.06f, 0.46f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.7f, 0.50f, 1.8f);
	...
	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(1.3f, 0.005f, 1.3f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.7f, .62f, 1.8f);
	...
	/*********************************************************
	/  Draw the rounded bottom of the plate
	/********************************************************/
	...
	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(1.4f, 0.09f, 1.4f);
	...
	ZrotationDegrees = -4.4f;

	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(1.1f, .78f, 2.2f);
